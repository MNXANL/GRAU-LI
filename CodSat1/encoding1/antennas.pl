...
%%%% Aux variables %%%%
stationOfCity(C, S) :- city(C, List), member(S, List).

station(S) :- numStations(NS), between(1, NS, S).
importantCity(C) :- importantCities(ICList), member(C, ICList).

%%%%%%%%%%%%%%%%%%%%%%%


%STC has a long list of possible antenna stations. 
%  Each city has a (non-empty) list of locations covering it.
%  Also, there's a list of important cities that must be covered by at least 2 stations. 
%  Overall, we can use at most [[maxStations]] antenna stations.
...
eachCityCoveredByAtLeastOneStation:-
	city(_, Antennas),
	findall(used-S, member(S, Antennas), Lits),
	atLeast(1, Lits),
	fail.
eachCityCoveredByAtLeastOneStation.

eachImportantCityCoveredByAtLeast2Stations:-
	importantCity(C),
	findall(used-S, stationOfCity(C, S), Lits),
	atLeast(2, Lits),
	fail.
eachImportantCityCoveredByAtLeast2Stations.
