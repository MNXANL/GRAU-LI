...
%  -trajects between nodes with the same parity have cost 1 euros !!!
%  -trajects between nodes  of different parity have cost 4 euros !!!
% For example, the following solution has cost 10:
%   1 10 4 8 3 20 18 14 12 11 15 2 5 9 17 16 7 13 19 6 1 
%    ^      ^ ^           ^     ^ ^      ^  ^       ^ ^
% We want to find a Hamiltonian cycle with cost at most maxCost.

% Complete the following program to compute one such Hamiltonian cycle.

%MANDATORY: Use the SAT var [visited-I-P] meaning "node I is visited in position P"
...
% Also use the SAT var [cost-C] meaning "the 2 nodes we've visited have cost C"
% 	More variables might be needed.
...
twoVisitedNodesHaveACost:-
	...
	writeClause([\+visited-N1-P1, \+visited-N2-P2, cost-P2-C]), 
	fail.
twoVisitedNodesHaveACost.


pathHasAtMostMaxCost:-
	findall(cost-P-C, (position(P), cost(_, C)), Lits), 
	maxCost(MVal),
	atMost(MVal, Lits),
	fail.
pathHasAtMostMaxCost.
...
displayCost(M):- 
	write('Cost = '), 
	findall(cost-P-1, (position(P), member(cost-P-1, M)), Lits1), length(Lits1, Cost1), 
	write(Cost1), write(' + '), 
	findall(cost-P-4, (position(P), member(cost-P-4, M)), Lits4), length(Lits4, Cost2aux),
	Cost2 is Cost2aux * 4, Cost is Cost1 + Cost2,
	
	nl, nl, write(Lits4), nl, nl,

	write(Cost2), write(' = '),
	write('['), write(Cost), write(']'), true.
displayCost(_):- nl.
