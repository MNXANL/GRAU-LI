...
% She has done it before with previous
% assignments and now, in order to diversify the composition of the
% groups, has a list of pairs of students that cannot be in the same
% group. 
% In addition, groups must have a minimum and a maximum number of members.


% DIFFERENCE BETWEEN writeClause() AND cardinality constraints
% --> writeClause()  es para
%
% --> CardConstraint es para 
%
...
forbidIncompatiblePairs:-
	incompatible(X, Y),
	group(G),
	writeClause([\+sg-X-G, \+sg-Y-G]),
	fail.
forbidIncompatiblePairs.
