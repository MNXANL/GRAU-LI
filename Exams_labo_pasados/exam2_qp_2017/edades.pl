...
%% - We want to plan all tasks (which gangster does what task when) and we want to find the minimal K such that no gangster works more than K consecutive hours.
...
gangstersNeeded( killing,       [4,1,   2,4,    2,1,    1,4,    1,1,    3,2,    4,2,    1,2,    1,3,    2,3,    4,1,    3,1,
                                 2,3,   1,3,    4,3,    2,3,    4,2,    3,1,    4,4,    1,4,    2,2,    1,4,    3,3,    3,2,
                                 2,3,   4,4,    1,3,    3,3,    4,4,    1,1,    2,3,    3,3,    3,2,    1,3,    1,1,    3,2] ).
                                 
gangstersNeeded( countingMoney, [1,2,   1,3,    1,4,    3,1,    3,1,    4,3,    2,2,    1,2,    1,2,    1,1,    2,1,    2,1,
                                 1,3,   1,2,    2,4,    3,2,    4,4,    4,1,    2,4,    4,2,    4,4,    4,3,    2,2,    1,3,
                                 2,1,   3,3,    2,3,    3,3,    1,4,    1,1,    3,1,    2,3,    3,1,    4,4,    3,3,    2,1] ).

gangstersNeeded( politics,      [2,4,   2,1,    1,1,    4,1,    1,4,    1,3,    2,4,    1,1,    4,1,    4,3,    1,3,    2,4,
                                 4,2,   4,2,    1,1,    4,3,    1,2,    2,2,    1,1,    3,1,    1,1,    2,2,    4,1,    1,3,
                                 4,4,   2,3,    2,4,    3,1,    1,1,    3,4,    2,2,    4,4,    3,1,    1,2,    1,4,    3,2] ).
...
%   2. work-G-H  means:  "gangster G works H hours (on model M)"

%% + No gangster can do two different tasks during the same hour ...
 % + ... or on two consecutive hours.
%% + Some gangsters are not available on certain hours.
%% + We want to plan all tasks (which gangster does what task when) and we want to find the minimal K such that no gangster works more than K consecutive hours.

%% Ayuda: puedes introducir variables involved-R-T que indican si el rango de edad R está involucrado en la tarea T

writeClauses(K):- 
    initClauseGeneration,              % OK [DEFAULT]
    eachHourEachGangsterAtMostOneTask, % OK
    makeKclauses(K),                   % OK
    planAllShitForMyMen,               % OK
    eachGangsterRestsBetweenTasks,     % OK
    eachGangsterAtMostTwoAgeRanges,
    true, !.
...
%%%%%%%% EXAM 2 %%%%%%%%
% Cada tarea sea realizada por como mucho dos franjas de edad. 
%
...
eachGangsterAtMostTwoAgeRanges :-
    ...
    age(G, A),
    ageRange(R1, X, Y),
    ageRange(R2, _, _),    R1 \= R2,
    ageRange(R3, _, _),    R3 \= R2, R1 \= R3,
    findall([\+involved-R1-T, \+involved-R2-T, \+involved-R3-T], (between(X, Y, A)), Lits ),
    atMost(2, Lits),
    fail.
eachGangsterAtMostTwoAgeRanges.
...
makeKclauses(K):-
    available(G, Hini),  task(T),
    Hfin is Hini + K,   %Window : H \in [Ini .. Fin]
    hour(K),
        
    age(G, A), ageRange(R, X1, Y1),    between(X1, Y1, A),
        
        
    findall( [\+does-G-T-H, involved-R-T], between(Hini, Hfin, H), Lit),
    writeClause(Lit),
    fail.
makeKclauses(_).

eachGangsterRestsBetweenTasks:- 
    between(1, 71, H1),   available(G, H1), 
    H2 is H1 + 1,   hour(H2),
...
planAllShitForMyMen :- %Formally, planAllTasks
    task(T),     hour(H),
    needed(T, H, N),
...
consec([H|[H2|T]]):-    H2 is H + 1,     consec([H2|T]).
consec([_]).
...
maxConsecutiveHours(M,K):-
    ...
    % Buscar para todas las horas las tareas del gangsta
    findall(H, member(does-G-_-H, M), List),

    % Mirar si L tiene una sublista de horas trabajadas de |L| = K 
    append(L, _, List),    append(_, Next, L),
    consec(Next),
    length(Next, K).
...
displaySol(M):- nl,nl,
    ...
    gangster(G), nl, 
    %write('gangster '), 
    write(G), write(': '), hour(H), writeIfBusy(G,H,M), fail,
   % write('Task killing: '),  nl,
   % write('Task countingMoney: '),  nl,
   % write('Task politics: '),  nl,
    !.
displaySol(_):- nl,nl,!.
...
treatResult(10,_):- %   shell('cat model',_),	
    ...
    maxConsecutiveHours(M,K), %To do: maxConsecutiveHours(M,K) /!\ /!\
